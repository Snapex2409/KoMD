...
void FENE::handleCell(Cell &cell) {
    SOA& soa = cell.soa();
    Kokkos::parallel_for("FENE - Cell", Kokkos::MDRangePolicy({0, 0}, {soa.size(), soa.size()}), FENE_Force(soa.fScatter(), soa.id(), soa.r(), soa.sigma(), soa.epsilon(), m_stiffness_factor));
}
